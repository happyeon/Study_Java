...
public class MTest03 {
	...
	private static void prn(char[] ch) {
		for (int i = 1; i <= ch.length; i++) {            // 1을 쓰는 이유는 1 % 0 = 0이기 떄문에 a 출력 후 줄이 바뀌기 때문
			System.out.printf("%c ", ch[i-1]);            // 0부터 쓰려면 cnt 함수 만들어서 쓰면됨
			if (i % 6 == 0) {
				System.out.println();
			}
		}
	...
	private static void upper(char[] ch) {
		// java.lang.Character 클래스에서 "대문자"로 바꾸는 메소드 사용!
